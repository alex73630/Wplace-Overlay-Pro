import {
	...
	LRUCache,
	overlayCache,
	overlayImageDataCache,
	paletteDetectionCache,
	tooLargeOverlays
} from "./cache"
import { blobToImage, canvasToBlob, createCanvas, loadImage } from "./canvas"
import { MAX_OVERLAY_DIM, MINIFY_SCALE, MINIFY_SCALE_SYMBOL, TILE_SIZE } from "./constants"
import { ensureHook, getUpdateUI } from "./hook"
import { SYMBOL_H, SYMBOL_TILES, SYMBOL_W, WPLACE_FREE, WPLACE_PAID } from "./palette"
...
function generateRLEData(imageData: ImageData, isPalettePerfect: boolean): RLEData {
	...
	for (let y = 0; y < height; y++) {
		...
		for (let x = 0; x < width; x++) {
			...
			const colorIndex =
				a <= 128 || (r === 0xde && g === 0xfa && b === 0xce)
					? -1
					: isPalettePerfect
						? (colorIndexMap.get(`${r},${g},${b}`) ?? -1)
						: findColorIndexLUT(r, g, b)
...
export async function buildOverlayDataForChunkUnified(
	...
	if (mode !== "minify") {
		...
		// --- RLE Optimized Minify Rendering ---
