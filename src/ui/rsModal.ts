...
import { canvasToDataURLSafe, createCanvas, createHTMLCanvas, loadImage } from "../core/canvas"
...
export function buildRSModal() {
  ...
  `
	...
	function drawSimplePreview() {
		...
		s.simOrig.width = leftW
		s.simOrig.height = leftH
		s.simNew.width = rightW
		s.simNew.height = rightH
	...
	function syncAdvancedMeta() {
		...
		if (s.mode === "advanced") {
			...
			const W = parseInt(s.w.value || "0", 10),
				H = parseInt(s.h.value || "0", 10)
			const ok =
				Number.isFinite(W) && Number.isFinite(H) && W > 0 && H > 0 && W < MAX_OVERLAY_DIM && H < MAX_OVERLAY_DIM
	...
	s.onex.addEventListener("click", () => {
		applyScaleToFields(1)
		drawSimplePreview()
	})
	s.half.addEventListener("click", () => {
		applyScaleToFields(0.5)
		drawSimplePreview()
	})
	s.third.addEventListener("click", () => {
		applyScaleToFields(1 / 3)
		drawSimplePreview()
	})
	s.quarter.addEventListener("click", () => {
		applyScaleToFields(1 / 4)
		drawSimplePreview()
	})
	s.double.addEventListener("click", () => {
	...
	s.calcBtn.addEventListener("click", async () => {
		...
		try {
			...
			if (cols >= MAX_OVERLAY_DIM || rows >= MAX_OVERLAY_DIM) {
				showToast(`Output too large. Must be < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM}.`)
				return
			}
		...
		try {
			if (s.mode === "simple") {
				...
				if (W >= MAX_OVERLAY_DIM || H >= MAX_OVERLAY_DIM) {
					showToast(`Too large. Must be < ${MAX_OVERLAY_DIM}×${MAX_OVERLAY_DIM}.`)
					return
				}
